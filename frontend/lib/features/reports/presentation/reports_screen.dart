            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                onPressed: () {
                  ...
                  try {
                    filters.validate();
                  } on ReportFilterException catch (error) {
                    setState(() {
...
class _ProjectDropdownState extends ConsumerState<_ProjectDropdown> {
  ...
  Future<void> _load() async {
    try {
      final dtos = await ref.read(projectRepositoryProvider).list();
      if (mounted) {
        setState(() {
          _projects = dtos;
          _loading = false;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return resultAsync.when(
      ...
      data: (result) => switch (result) {
        ProjectStatusReport() => _ProjectStatusResults(data: result.items),
        FinancialSummaryReport() => _FinancialSummaryResults(data: result),
        ExpenseAnalysisReport() => _ExpenseAnalysisResults(data: result),
      },
    );
  }
}
    ...
    return Column(
      ...
      children: [
        ...
        ...items.map((item) {
          ...
          final progress = item.progressBasisPoints / 10000.0;
          return Container(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(_formatPercent(item.progressBasisPoints),
                          style: TextStyle(
                              fontSize: 12,
                              fontWeight: FontWeight.bold,
                              color: statusColor)),
                  ...
                  Wrap(
                    ...
                    children: [
                      _MiniStat(
                          ...
                          value: _formatMoney(
                              item.budgetAmountMinor, item.budgetCurrency),
...
class _FinancialSummaryResults extends StatelessWidget {
  ...
  final FinancialSummaryReport data;
...
class _FinancialProjectRow extends StatelessWidget {
  ...
  final ProjectFinancialReportRow project;

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isPositive = project.netCashFlowYer >= 0;
    return Container(
...
class _ExpenseAnalysisResults extends StatelessWidget {
  ...
  final ExpenseAnalysisReport data;
    ...
    return Column(
      ...
      children: [
        _KpiCard(
          ...
          value: _formatYer(data.grandTotalYer),
          color: Colors.orange,
          icon: Icons.receipt_long_outlined,
        ),
        const SizedBox(height: 20),
...
class _CategoryBar extends StatelessWidget {
  ...
  final ExpenseCategoryReportRow item;
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(_formatPercent(item.shareBasisPoints),
                  style: const TextStyle(
          ...
          ClipRRect(
            borderRadius: BorderRadius.circular(4),
            child: LinearProgressIndicator(
              value: (item.shareBasisPoints / 10000.0).clamp(0.0, 1.0),
              minHeight: 7,
          ...
          const SizedBox(height: 4),
          Text(_formatYer(item.totalExpensesYer),
              style: TextStyle(
                  fontSize: 12,
...
class _ProjectExpansionTile extends StatelessWidget {
  ...
  final ProjectExpenseReportRow project;
    ...
    return Container(
      ...
      child: ExpansionTile(
        ...
        title: Text(project.name,
            style: const TextStyle(fontWeight: FontWeight.bold)),
        trailing: Text(_formatYer(project.totalExpensesYer),
            style: const TextStyle(
                fontWeight: FontWeight.bold, color: Colors.orange)),
        children: project.byCategory
            .map((c) => ListTile(
                  ...
                  trailing: Text(
                      '${_formatYer(c.totalExpensesYer)}  (${_formatPercent(c.shareBasisPoints)})',
                      style: TextStyle(
                          color: theme.colorScheme.onSurface
                              .withValues(alpha: 0.65))),
                ))
...
String _netCashFlowLabel(BuildContext context) =>
